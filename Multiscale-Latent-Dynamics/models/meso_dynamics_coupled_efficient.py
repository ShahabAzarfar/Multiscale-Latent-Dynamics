...
class MesoDynamicsCoupledEfficient(keras.Model):
    """Learn the vae-encoded coupled evolution of all physical fields together over the latent space at meso-scale.
    ...
    def __init__(self, meso_ts, meso_micro_ts_ratio, fields, output_fields, latent_shape, loss_weights, dynamics_model):
        """
        Args:
            meso_ts (int):   number of predicted meso-scale time-steps
        ...
        self.meso_ts = meso_ts
        self.meso_micro_ts_ratio = meso_micro_ts_ratio
        self.fields = fields
        self.output_fields = output_fields 
        self.latent_shape = latent_shape  # (40, 60, 4) for meso-scale data
    ...
    ###
    def micro_to_meso_evolution(self, in_z_mean, in_z_var):
        """Apply the full autoregressive functional to the latent-mean and latent-log-vaiance fields for one meso-scale time-step.
        
        Considering the ratio of meso-scale to micro-scale time-steps, the autoregressive functional needs 
        to be applied repeatedly according to `meso_micro_ts_ratio` factor.
        ...
        Return:
            ...
            evol_z_var (tf.Tensor):    evolved latent-log-variance field of shape (N, H, W, channels)
        """
        current_z_mean, current_z_var = in_z_mean, in_z_var

        for _ in range(self.meso_micro_ts_ratio):
    ...
    def call(self, input):
        ...
        Args:
            input (list):   list of the form [init_z_mean, init_z_var, evol_step, output_fields], where
                            ...
                            meso_evol_step (int):    number of predicted meso-scale time-steps
                            output_fields (list of 2-tuples):   list of the form [(field_idx, field_name)] specifying the physical fields
        ...
        """
        init_z_mean, init_z_var, meso_evol_step, output_fields = input[0], input[1], input[2], input[3] 
        z_mean_traj = [None] * (meso_evol_step + 1)
        z_var_traj = [None] * (meso_evol_step + 1)
        ...
        for t in range(1, meso_evol_step + 1):
            z_mean, z_var = self.micro_to_meso_evolution(z_mean, z_var)                                                                           
        ...
        x_evol_traj = [None] * (meso_evol_step + 1)
        for t in range(meso_evol_step + 1):
    ...
    ###
    def predict_wo_sampling(self, input):
        """Deterministic version of the Call function in which we only use z_mean as the input for decoder"""
        ...
        for t in range(meso_evol_step + 1):
            ...
            for i, f in enumerate(output_fields):
                ...
                z_mean = z_mean_traj[..., c0:c1, t]
                z_var = z_var_traj[..., c0:c1, t]
                latent_field = z_mean   # latent_field = Sampling()([z_mean, z_var]) in the original `Call` function
                field_evol = getattr(self, f'{field}_decoder')(latent_field)
                x_evol[i] = field_evol
            x_evol = tf.concat(x_evol, axis=3)
            x_evol_traj[t] = x_evol
